    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content border-0">
            <div class="modal-header bg-info text-white">
                <h5 class="modal-title text-white">해당 친구의 미니홈페이지에 놀러가보세요!</h5>
                    ...
                    <form action="javascript:void(0);" id="addnotesmodalTitle">
                        <div class="row">
                            <div class="col-md-12 mb-3">
                                ...
                                <label>아이디 목록</label>
                                <input type="text"  class="form-control" minlength="25" placeholder="Id" />
                                </div>
                            </div>
                        </div>
                    </form>
...
{{--                             <div class="timeline-btn">
                                <a href="#" class="btn btn-primary waves-effect waves-light m-r-10">Add Friends</a>
                            ...
                            <div class="social-timeline-left">
                                <div class="card">
                                    ...
                                    <div class="card-block social-follower">
                                        <h4>{{ Auth::user()->name }} 님의 펫 {{$user->pet_name}}</h4>
                        ...
                        <div class="col-xl-9 col-lg-8 col-md-8 col-xs-12 ">
                            ...
                            <div class="card social-tabs">
                                <ul class="nav nav-tabs md-tabs tab-timeline" role="tablist">
                                    ...
                                    <li class="nav-item">
                                        <a class="nav-link" data-toggle="tab" href="#about" role="tab">Friends</a>
                                        <div class="slide"></div>
                                    </li>
                                    <li class="nav-item">
											                            ...
											                            <div class="card card-white grid-margin" id="post-full-container">
                                                                            ...
                                                                            <div class="guestbook-container-{{$guest_book[0]->id}}">
                                                                                ...
                                                                                @endforeach
                                                                                    ...
                                                                                    <div class="media">
                                                                                        <a class="media-left" href="#">
                                                                                            <img class="editable img-responsive" alt=" Avatar" id="avatar2" @if($guest_book[0]->user_asset_id != 0) src="{{url('/assets/img/profile/'.$guest_book[0]->user_asset_file_name.'.'.$timeline[0]->user_asset_file_extension)}}" @else src="{{ url("/assets/mononoke111.jpg") }}" @endif style="width: 50px; height: 50px;">
                                                                                        </a>
                                                                                        <div class="media-body">
                                                                                            <form class="">
                                                                                                <div class="">
...
<script type="text/javascript">
    ...
    function addTimeline(){
        ...
        function addTimelineComment(timeline_id){
            ...
            mini_ajax("post","/homepage/add-timeline-comment",ajax_data,function(resp){
                $(".timeline-container-"+timeline_id).append(resp);
    ...
    function deletePhoto(elem){
        var photo_id = $(elem).parents(".card").attr('data-id');
        ...
        if(confirm("해당 앨범을 정말 지우시겠습니까?")) {
            mini_ajax("put",'/homepage/delete-photo',ajax_data,function(resp){
                $(elem).parents('.card').remove();
            })
        }
    }

    function deleteTimeline(elem){
        var timeline_id = $(elem).parents(".card").attr('data-id');
        console.log('timeline_id',timeline_id);
        ...
        if(confirm("해당 타임라인의 게시물을 정말 지우시겠습니까?")){
            mini_ajax("put","/homepage/delete-timeline",ajax_data,function(resp){
                $(elem).parents('.card').remove();
            })
        }
    }

    function deleteTimelineComment(elem){
        var timeline_id = $(elem).parents(".timeline-comment").attr('data-id');
        console.log(timeline_id);

        var ajax_data={
            'timeline_id' : timeline_id 
        }
        if(confirm("해당 댓글을 정말 지우시겠습니까?")){
            mini_ajax("put",'/homepage/delete-timeline-comment',ajax_data,function(resp){
                $(elem).parents('.timeline-comment').remove();
            })
        }
    ...
    function deleteGuestbook(elem){
        var guestbook_id = $(elem).parents(".timeline-item").attr('data-id');

        var ajax_data={
            'guestbook_id' : guestbook_id
        }

        if(confirm("해당 방명록의 글을 정말 지우시겠습니까?")){
            mini_ajax("put",'/homepage/delete-guestbook',ajax_data,function(resp){
                $(elem).parents('.timeline-item').remove();
            })
        }
    }
    ...
    function addGuestbookComment(guestbook_id){
        ...
        mini_ajax("post","/homepage/add-guestbook-comment",ajax_data,function(resp){
            $(".guestbook-container-"+guestbook_id).append(resp);
            confirm("해당 방명록에 댓글이 게시되었습니다.");

        })

    }
        function deleteGuestbookComment(elem){
            var guestbook_id = $(elem).parents(".timeline-comment").attr('data-id');
            console.log(guestbook_id);
        ...
        var ajax_data={
            ...
            if(confirm("해당 댓글을 정말 지우시겠습니까?")){
                mini_ajax("put",'/homepage/delete-guestbook-comment',ajax_data,function(resp){
                    $(elem).parents('.timeline-comment').remove();
                })
            }
    }
    ...
    function searchFriends(elem){
        ...
        console.log("friend_id : ",friend_id);
        mini_ajax("post",'/homepage/search-friends',ajax_data,function(resp){
            if(resp.status=="success"){
                alert("해당 아이디의 사용자의 미니 홈페이지에 놀러가보세요!");
                window.location.href = "{{ env("MINI_HOMEPAGE_URL").'homepage/friend-main/' }}" + resp.user.id
            }else{
